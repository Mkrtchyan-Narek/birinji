...
export default function Chat() {
  ...
  const quitGame = () => {
    ...
    newPlayers.pop();
    updateDoc(doc(db, "games", gameInfo.id), {players: newPlayers, deck_id: deckId});
  ...
  async function createGame() {
    ...
    const game = await addDoc(collection(db, "games"), gameData);
    const ids = [game.id];
    updateDoc(doc(db, "games", "openGames"), {ids});
  ...
  const handleKeyDown = async (event) => {
    ...
    else if(gameInfo.left_cards == 16 && playedCards.length == 0 && getHandValue(cards[me].map(card => card.code[0])) == 0 && event.code == "KeyX") {
      var newDoc ={changed: !gameInfo.changed};
      newDoc.winner = me%2 == 0 ? 1 : 0;
      newDoc.cards = JSON.stringify([[], [], [], []]);
      newDoc.played_cards = JSON.stringify([]);
      newDoc.left_cards = 0;
      updateDoc(doc(db, "games", gameInfo.id), newDoc);
    }
  };
  ...
  React.useEffect(() => {
    (async function asyncFunc() {
      ...
      if(currentGame) {
        let currentGameData = (await getDoc(doc(db, "games", currentGame))).data();
      ...
      let newDoc={};
      let openGames = (await getDoc(doc(db, "games", "openGames"))).data().ids;
      if(!openGames.length && game == "none") {
        ...
        game = openGames[0];
        gameData = (await getDoc(doc(db, "games", game))).data();
      }

      const q = query(collection(db, "games"), where("open", "==", false));
      ...
      querySnapshot.docs.forEach((currentDoc) => {
        if(!JSON.parse(currentDoc.data().cards).some(x => x.length > 0) && currentDoc.data().left_cards == 0) {
          deleteDoc(doc(db, "games", currentDoc.id));
        }
      });
      ...
      let newPlayers = gameData.players.includes(user.uid) ? gameData.players : gameData.players.length < 4 ? [...gameData.players, user.uid] : gameData.players.map((val, i) => i == gameData.players.indexOf("bot") ? user.uid : val);
      newDoc={players: newPlayers, deck_id: currentDeckId}
      setPlayers(newDoc.players);
      if(gameData.players.filter(player => player != user.uid).length == 3 && gameData.open == true) {
        setGameInfo({...gameData, id: game, open: false});
        const newIds = openGames.filter(id => id != game);
        updateDoc(doc(db, "games", "openGames"), {ids: [...newIds]});
        newDoc.open=false;
        newDoc.left_cards = 16;
        newDoc.cards = await drawBase(currentDeckId, game, newDoc.players);
      }
      updateDoc(doc(db, "games", game), newDoc);
      const unsub = onSnapshot(doc(db, "games", game),async (snapshot) => {
  ...
  React.useEffect(() => {
    if(announcements[announcements.length-1]) {
      ...
      if(announcements[announcements.length-1].player == me) {
        ...
        updateDoc(doc(db, "games", gameInfo.id), newCount);
      }
...
function leave() {
  if(turn == me) {
    dispatch(
      setSnackbar({
      show: true,
      message: "Play a card before leaving!",
      type: "error",
      sx: {
        position: "fixed",
        top: "-20%",
...
async function playBot(botId, newCards, newPlayedCards) {
  ...
  let next = botId;
    ...
    const duos = [['QS', 'KS'], ['QH', 'KH'], ['QD', 'KD'], ['QC', 'KC']];
    const cardCodes = new Set(newCards[botId].map(card => card.code));
    const [s, h, d, c] = duos.map(([card1, card2]) => (cardCodes.has(card1) && cardCodes.has(card2)));
    ...
    updateDoc(doc(db, "games", gameInfo.id), {combinations: JSON.stringify(newAnnouncements)});
  ...
  updateDoc(doc(db, "games", gameInfo.id), {cards: JSON.stringify(newCards), played_cards: JSON.stringify(newPlayedCards), turn: (next+1)%4});
  if(newPlayedCards.length == 4) {
    ...
    if(windex % 2 == 0) {
      ...
      updateDoc(doc(db, "games", gameInfo.id), {count_1: score1+hand});
    } else {
      setScore2(score2+hand);
      updateDoc(doc(db, "games", gameInfo.id), {count_2: score2+hand});
    }
    setplayedCards([]);
    ...
    updateDoc(doc(db, "games", gameInfo.id), newDocForHand);
    if(players[windex] == "bot") {
      playBot(windex, newCards, newPlayedCards);
    }
    }, 1000);
  } else if(players[(botId+1)%4] == "bot") {
  ...
  return (
    <img 
      ...
      onClick={(turn == me) && playedCards.length != 4 && (hasJustAnnounced != "none" && playedCards.length > 0 ? (hasJustAnnounced == card.code[1] && (card.code[0] == "Q" || card.code[0] == "K")) : true) ? async () => { 
        ...
        setCards(newCards);
        setplayedCards(newPlayedCards);
        updateDoc(doc(db, "games", gameInfo.id), {turn: next, cards: JSON.stringify(newCards), played_cards: JSON.stringify(newPlayedCards)});
        ...
        >
          ...
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("H")) {if(announcements.length==0 && turn == me) {
              ...
              updateDoc(doc(db, "games", gameInfo.id), {combinations: JSON.stringify([...announcements, {player: me, suit: "H"}])});
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("S")) {if(announcements.length==0 && turn == me) {
              ...
              updateDoc(doc(db, "games", gameInfo.id), {combinations: JSON.stringify([...announcements, {player: me, suit: "S"}])});
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("D")) {if(announcements.length==0 && turn == me) {
              ...
              updateDoc(doc(db, "games", gameInfo.id), {combinations: JSON.stringify([...announcements, {player: me, suit: "D"}])});
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("C")) {
              ...
              updateDoc(doc(db, "games", gameInfo.id), {combinations: JSON.stringify([...announcements, {player: me, suit: "C"}])});
