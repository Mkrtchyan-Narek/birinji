...
export default function Chat() {
  ...
  const [gameInfo, setGameInfo] = React.useState({players:[]});
  ...
  React.useEffect(() => {
    (async function asyncFunc() {
      ...
      if(gameData.players.length == 3) {
        ...
        setMyCards(JSON.stringify(newDoc.cards[gameData.players.length]));
      }
      ...
      const unsub = onSnapshot(doc(db, "games", game),async (snapshot) => {
        ...
        let me = gameInfoNew.players.indexOf(auth.currentUser.uid);
        ...
        if(gameInfoNew.left_cards == 0 && !JSON.parse(gameInfoNew.cards).some(arr => arr.length != 0)) {
  ...
  function getBiggestCard(currentCards) {
...
function PlayerCard({id, index, angle}) {
  ...
  return (
    <img 
      key={index} 
      src={card.image}
      onClick={(turn == me) ? async () => {        
        setplayedCards([...playedCards, {...card, player: me}]);
        updateDoc(doc(db, "games", gameInfo.id), {played_cards: JSON.stringify([...playedCards, {...card, player: me}])});
        let newCards = [...cards];
        newCards[me] = [...cards[me].filter(currentCard => currentCard.code != card.code)];
        setCards(newCards);
        updateDoc(doc(db, "games", gameInfo.id), {cards: JSON.stringify(newCards), turn: (me+1)%4});
    ...
    width: "100%",
        ...
        >
          ...
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("H")) {
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("S")) {
  ...
  <div style={{ 
    ...
    width: "100%",
        ...
        <ListItem disablePadding>
          <ListItemButton onClick={() => {
            if((playedCards.length == 0 || turn == me) && !announcements.map(announcement => announcement.suit).includes("D")) {
